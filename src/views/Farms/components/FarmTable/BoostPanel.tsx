...
const baseUrl =
  "https://bafybeidcbdmm6pzmravo6mwygqdmxxoexbrmsoomln7ume3i75at26w6sq.ipfs.nftstorage.link";
const baseUrl1 =
  "https://bafybeigcxihqs7vtrfdxzdy36qh4j26vk7yixq3csj5z7otjeerfrcnhoi.ipfs.nftstorage.link";
...
const BoostPanel: React.FunctionComponent<BoostPanelProps> = ({ pid, isApproved, tokenIds, boost }) => {
  ...
  const [onBoost1] = useModal(
    <BoostModal pid={pid} slot={1}/>,
  )
  const [onBoost2] = useModal(
    <BoostModal pid={pid} slot={2}/>,
  )
  const [onBoost3] = useModal(
    <BoostModal pid={pid} slot={3}/>,
  )
    ...
    <Container>
      ...
      <NFTContainer>
        {[1, 2, 3].map((e) => (
          <NFTBox
            key={e}
            disabled={approvePending || withdawPenidng}
            onClick={() => onBoxClick(e)}
          >
            {
              tokenIds && tokenIds[e - 1] ?
              <Tooltip
                content={`Token ID: ${tokenIds[e - 1]}`}
              >
